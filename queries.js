const pgClient = require('./dbconnection'); // Deine bestehende Datenbankverbindung importieren
...
async function checkObjektIDExists(objektid) {
    ...
    const { rows } = await pgClient.query(query, [objektid]);
...
async function insertWohnung(objektid, verfuegbar_ab, baujahr) {
    ...
    await pgClient.query(query, [objektid, verfuegbar_ab, baujahr]);
}
...
async function insertKontaktperson(objektid, name, email, telephone) {
    ...
    await pgClient.query(query, [objektid, name, email, telephone]);
}
...
async function insertFlachen(objektid, anzahlzimmer, wohnflaeche, balkon) {
    ...
    await pgClient.query(query, [objektid, anzahlzimmer, wohnflaeche, balkon ? 1 : 0]);
}
...
async function insertAusstattung(objektid, aufzug, balkon) {
    ...
    await pgClient.query(query, [objektid, aufzug ? 1 : 0, balkon ? 1 : 0]);
}
...
async function insertPreise(objektid, heizkosten, nebenkosten, kaltmiete, nettokaltmiete, freitext_preis) {
    const query = `
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
    `;
    await pgClient.query(query, [objektid, heizkosten, nebenkosten, kaltmiete, nettokaltmiete, freitext_preis]);
}
...
async function insertFreitexte(objektid, objekttitel, lage, ausstattungsbeschreibung) {
    const query = `
        ...
        VALUES ($1, $2, $3, $4)
    `;
    await pgClient.query(query, [objektid, objekttitel, lage, ausstattungsbeschreibung]);
}
...
async function insertGeo(objektid, strasse, hausnummer, plz, ort, etage) {
    const query = `
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
    `;
    await pgClient.query(query, [objektid, strasse, hausnummer, plz, ort, etage]);
}
